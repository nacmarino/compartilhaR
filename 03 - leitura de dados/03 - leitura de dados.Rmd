...
* Importe os arquivos `autores.xls` e `revistas.xlsx` e armazene cada um deles em um objeto distinto.
* A importação ocorreu sem problemas?
* Qual o tamanho das dimensões de cada um destes objetos?
* Selecione apenas a segunda coluna da tabela `revistas.xlsx`.
* Selecione apenas as linhas 10 a 40 da tabela `revistas.xlsx`.
* Exclua apenas a primeira coluna da tabela `revistas.xlsx`.
...
* Do objeto onde você armazenou a tabela de dados `revistas.xlsx`, retenha apenas as observacoes que correspondam às revistas brasileiras, e armazene o resultado dessa operação em um novo objeto.
* Exporte este objeto, para o diretório e utilizando a extensão de sua escolha.
...
* Com os dois `data.frame` abaixo, tente juntá-los em uma único objeto utilizando a função `rbind.data.frame` e list`.
...
* Crie uma lista contendo os dados presentes em `dados dos projetos.csv`, `autores.xls`, `publicacoes.xls` e `revistas.xls`.
* Salve esta lista em algum diretório no seu computador.  
* Importe o arquivo contendo esta lista novamente para o R, e atribua ela a um objeto.  
* Selecione o terceiro elemento da lista que você importou.  
